...
export default function ContactForm() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload: Record<string, string> = {
            ...
            _replyto: 'nanoset@naver.com',
            _cc: 'nanoset@naver.com',
        };
    ...
    if (status === 'success') {
        return (
            <div className="bg-white p-8 sm:p-12 rounded-3xl text-center border-2 border-[#E8941A] shadow-lg">
                ...
                <p className="text-gray-500 font-medium mb-8 text-lg">이의환 후보가 직접 확인합니다. 소중한 의견 감사합니다.</p>
    ...
    return (
        <form onSubmit={handleSubmit} className="bg-white p-6 sm:p-10 rounded-[1.5rem] space-y-6 m-1">
            ...
            <div className="space-y-4">
                ...
                <Textarea name="message" required placeholder="이의환 후보에게 전할 응원이나 건의사항을 남겨주세요." rows={5} value={formData.message} onChange={handleChange} className="rounded-2xl bg-gray-50 border-gray-200 focus:bg-white p-6 font-medium text-lg resize-none focus:border-[#1B3A6B] focus:ring-0 transition-colors" />
            </div>
